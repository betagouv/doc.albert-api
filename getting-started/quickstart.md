...
Créez une clé API avec `POST /v1/me/keys` (voir [Clés API](../account/api-keys.md)). Exportez-la dans votre environnement, par exemple :
...
Choisissez un modèle dont le champ `type` est **`text-generation`** (voir [Types de modèles](../models/model-types.md)). Notez son `id` (par ex. `Meta-Llama-3.1-8B-Instruct` — l’offre exacte dépend de la plateforme).
...
Pour aller plus loin : [Chat completions](../guides/chat-completions.md), [Référence OpenAPI](../api-reference/reference.md).
