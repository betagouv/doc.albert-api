# Outil de recherche RAG intégré
...
{% hint style="warning" %}
⚠️ Les anciens exemples qui utilisaient des champs du type `collections` / `prompt` au lieu de `collection_ids` / `query`, ou un chemin `/search` sans préfixe `/v1`, sont **obsolètes** : alignez-vous sur la [Référence OpenAPI](/broken/pages/vRZwVxl3vzeW4GecHnw7).
{% endhint %}

{% hint style="warning" %}
⚠️ À vérifier — Détails JSON exacts des filtres `metadata_filters` et comportement lorsque `collection_ids` et `document_ids` sont vides : se reporter aux schémas `ComparisonFilter`, `CompoundFilter` et `SearchTool` dans la [Référence OpenAPI](/broken/pages/vRZwVxl3vzeW4GecHnw7).
{% endhint %}
